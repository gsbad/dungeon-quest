...
| `professions.py` / `class_kits.py` / `spells.py` / `items.py` / `affixes.py` / `difficulty.py` / `weather.py` / `status_effects.py` / `stances.py` | Sistemas de RPG independentes entre si - cada um é essencialmente uma tabela (`dict`) + uma função pequena que lê a tabela, sem hierarquia de classes. `class_kits.py` liga cada uma das 15 profissões ao seu ataque básico (multiplicadores sobre o hitbox universal) e trio de magias (`spells.py` guarda a definição das 47 magias, `class_kits.py` decide QUEM pode conjurar QUAL); "ter a profissão" é o único requisito - não existe mais atributo-gate por magia. `status_effects.py` cobre debuffs (Veneno, Fogo...), buffs temporários de poção/elixir/magia, e os aplicados pelo jogador num inimigo (Lentidão de área, Atordoamento, Provocação) - mesmo `StatusEffectDef`/`StatusEffectCarrier` pros três casos. `stances.py` é a única tabela *permanente* (não expira) - uma Postura é derivada direto da profissão atual, igual a profissão já é derivada dos atributos gastos. |
...
| `net_coop.py` | Conexão WebSocket do modo cooperativo (`/coop/ws/{room_id}` no backend) - mesma ponte pyodide/emscripten ↔ nativo que `net.py` já resolve pro resto do jogo, só que persistente (não fire-and-forget) e com uma fila de mensagens recebidas (`poll_messages()`). Host-autoritativo pra combate: um guest calcula seu próprio dano mas manda um `"hit_request"` em vez de aplicar local (só o host chama `take_damage()` de verdade em `Enemy`/`Boss`, ver `level.py`/`states.py`), e o broadcast de `"enemies"` (já ~12Hz) devolve o resultado. Quebra de bloco/chave escondida (`Level._break_block()`/`_pick_key_tile`) já são **simétricas** em vez de host-autoritativas - sem risco real de conflito, qualquer lado que completar a ação avisa o outro (`"block_broken"`/`"key_found"`). |
| `coop_overlay.py` / `remote_player.py` / `chat_widget.py` | UI/entidades do cooperativo - `CoopOverlay` é o painel de criar/entrar numa room (código curto, sem login), `RemotePlayer` é o avatar só-visual de outro jogador na mesma room (posição interpolada, nunca simula nada), `ChatWidget` é o campo de texto livre do chat (Enter abre/envia, Esc cancela). |

**Por que o backend duplica alguns defaults do jogo** (`BALANCE_DEFAULTS`, contagem de conquistas): o deploy do backend não tem acesso ao pacote `game/` (são dois deploys/processos separados, o backend nem importa pygame). Em vez de criar uma dependência cruzada esquisita, os poucos valores que o backend precisa saber (preços, custos de magia, stats de monstro, efeitos de buff/debuff, bônus de postura, o que conta como conquista) são pequenas reimplementações somente-leitura, deliberadamente - `BALANCE_DEFAULTS` tem ~850 chaves pontilhadas (`"monster.goblin.strength"`, `"spell.raio_de_gelo.mana_cost"`, `"buff.buff_orange.physical_damage_mult"`, `"stance.Guerreiro.physical_damage_mult"`...), geradas diretamente a partir dos dicts reais do jogo (não digitadas à mão) para não desviar dos valores de verdade.
...
- **`/coop/room` + `/coop/ws/{room_id}`**: cooperativo online. `POST /coop/room` gera um código curto de 4 caracteres; `/coop/ws/{room_id}` é um relay burro em memória (`dict[room_id -> {player_id: {ws, name, is_host}}]`) - o backend nunca inspeciona o conteúdo semântico das mensagens além de carimbar `player_id` (server-stamped, nunca um campo que o próprio remetente poderia forjar) e rebroadcastar pro resto da room (`_coop_broadcast(..., exclude=remetente)`). Sem persistência nenhuma - uma room morre quando o último jogador sai, sem tocar no banco.
...
`tools/coop_harness.py` é o equivalente para o cooperativo - sobe o backend + build pygbag localmente e coordena **dois** `BrowserContext` do Playwright ao mesmo tempo (host e guest, cada um seu próprio WebSocket), algo que a disciplina de "um browser + screenshot" acima não cobre. Confirma via tráfego de rede real (frames de WebSocket inspecionados diretamente, não só screenshot) cada etapa - entrar na mesma room, sincronizar posição/inimigos, dano PvP, jogador "caído"/revive, os debuffs/bônus de PvP amigável, chat - e reusa a mesma sessão entre etapas em vez de recriar do zero a cada uma. A sequência de boot/join comum vive em `_connect_coop_session()`, reusada por `run_test()` (a sessão original acima) e por `run_world_sync_test()` - uma segunda sessão independente que confirma que quebra de bloco e dano de guest contra um `Enemy` de fato chegam no outro lado (`"block_broken"`/`"hit_request"` observados no tráfego real, ver "Correções pós-playtest coop" em `design.md`).

`tools/soak_test.py`/`tools/coop_soak_test.py` cobrem a categoria "trava depois de uma sessão longa" que um teste de alguns segundos não consegue reproduzir por construção - rodam minutos de atividade contínua (o segundo dentro de uma room coop de verdade, via `coop_harness.py`) enquanto amostram o RSS real do processo renderer do Chromium (`SystemInfo.getProcessInfo` via CDP + `/proc/<pid>/status` - `performance.memory.usedJSHeapSize` sozinho não basta, só cobre o heap do V8, não a memória linear do módulo WASM onde o Python/pygame de verdade vive). Ver a seção "Travamento de sessão estendida" em `design.md` pro que já foi achado com eles.
