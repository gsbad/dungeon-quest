...
| `net_coop.py` | Conexão WebSocket do modo cooperativo (`/coop/ws/{room_id}` no backend) - mesma ponte pyodide/emscripten ↔ nativo que `net.py` já resolve pro resto do jogo, só que persistente (não fire-and-forget) e com uma fila de mensagens recebidas (`poll_messages()`). Host-autoritativo: só o host simula `Enemy`/`Boss` de verdade (`network_follower=True` em quem não é host, ver `level.py`/`enemy.py`/`boss.py`), guests só recebem snapshots. |
