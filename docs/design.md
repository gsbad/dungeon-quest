...
2 tipos de linha:
- **Ajuste** (atributos ±5, nível ±1, pontos livres ±1, ouro ±50, **kills totais ±10, mortes totais ±1** — ver abaixo —, dificuldade atual — circula livre pelas 5, ignorando o desbloqueio sequencial, que é o próprio propósito da ferramenta).
...
**Poções: uma lista só, no fim do painel (bugfix round pós-coop).** As 23 entradas de item (3 poções base + 20 poções/elixires com buff) eram DUAS listas redundantes — uma linha "Ajuste" por item que só mexia em `player.inventory`, e uma linha "Trigger" separada por buff que só chamava `player.status.apply()`, sem tocar o inventário. Consolidadas num único `_potion_row()` por item com buff (linha "Trigger" que dá +1 no inventário e chama `use_item()` — consome a unidade e aplica o efeito na mesma tecla, exercitando o mesmo caminho que o botão "Usar" do menu Itens usa de verdade), e movidas pro último bloco do painel, logo antes da linha de status de login/sync (que continua sendo a última linha absoluta).
...
## Modo cooperativo (Stage L1-L14)

Ver `docs/coop-feasibility.md` (decisões de arquitetura) e `docs/coop-implementation-plan.md` (ordem de construção, sub-estágio por sub-estágio) pro histórico completo. Resumo do que existe hoje:

- **Entrar numa room**: um código curto de 4 caracteres (`POST /coop/room` no backend), sem exigir login - o nome mostrado é o `player.name` do save local, mesmo princípio "login nunca é obrigatório" do resto do jogo.
- **Host-autoritativo**: só quem criou a room simula `Enemy`/`Boss` de verdade (IA, dano, drops) - todo mundo mais recebe snapshots de posição/HP a ~1x/s e só desenha (`network_follower=True`, ver `game/level.py`/`enemy.py`/`boss.py`). Entrar numa room coop no MEIO de uma fase já carregada só liga o modo rede a partir da PRÓXIMA transição de fase (limite conhecido da v1). **Guest também consegue ferir/matar inimigos e blocos/chave também sincronizam** — ver "Correções pós-playtest coop" abaixo, achados reais de um playtest com 2 jogadores de verdade que a v1 (L1-L14) não cobria.
- **XP/ouro compartilhado**: XP de qualquer kill é dividido igualmente entre todos na room (não proporcional a quem bateu o golpe final); ouro de inimigo comum continua um `GoldDrop` físico não-compartilhado (quem chega primeiro pega, igual single-player), mas ouro/XP de boss são creditados igualmente pros dois na hora.
- **"Caído" e fim de partida**: chegar a 0 HP em coop não reinicia a fase (diferente do single-player) - o jogador fica "caído" 5s e revive sozinho com metade do HP máximo. Se todos os jogadores da room caírem ao mesmo tempo, a partida acaba e todo mundo volta pro menu.
- **PvP amigável** (dano jogador-contra-jogador ligado, com consequência de verdade em vez de simplesmente desligado):
  - **Traiçoeiro**: -15% em todos os stats de combate por 10s, aplicado em quem ACERTA um aliado.
  - **Homicida**: -35% por 2min, aplicado em quem DERRUBA (0 HP) um aliado - bem mais forte/longo que Traiçoeiro.
  - **Reação Justa**: +15% de dano por 10s, concedido à VÍTIMA especificamente contra quem acabou de acertá-la (não a room inteira) - primeiro uso de um bônus *dirigido* jogador→jogador no jogo (`Player.vengeance`, `game/status_effects.py`'s `DirectedBonusCarrier`), diferente de todo buff/debuff anterior (que só descrevem "meu stat X está Y% melhor", nunca contra um alvo específico).
  - **Acerto de Contas**: +35% por 2min, versão mais forte de Reação Justa quando o golpe foi letal (derrubou a vítima).
- **Chat por balão de fala**: `Enter` abre um campo de texto livre (PC apenas - sem teclado virtual/mobile ainda), a mensagem enviada aparece como balão acima do herói por 3.5s, tanto pro próprio jogador quanto pros outros na room (transportada no mesmo canal WebSocket de posição/combate, sem persistência nenhuma).
- **Testado com `tools/coop_harness.py`** (ver `docs/architecture.md`) - dois `BrowserContext` reais coordenados no mesmo teste, confirmando via tráfego de WebSocket real (não só screenshot) cada mecânica acima.
...
## Travamento de sessão estendida, segunda rodada (Stage L1-L14 pós-coop)

O primeiro travamento desse tipo (ver `font()` em `game/theme.py`) foi diagnosticado e corrigido há tempo: `pygame.font.Font` sendo reconstruído do zero a cada chamada, em dezenas de pontos de `draw()` chamados todo frame, degradava aos poucos o heap WASM/emscripten (que cresce e raramente encolhe) até travar de vez, sem erro nenhum no console. O usuário reportou o mesmo sintoma persistindo mesmo depois - tanto desktop quanto mobile - já com o cooperativo em uso real. Investigado ao vivo com dois novos harnesses de "soak test" (`tools/soak_test.py`, single-player; `tools/coop_soak_test.py`, dois `BrowserContext` reais numa room coop), ambos rodando minutos de atividade contínua enquanto amostram memória de verdade - `performance.memory.usedJSHeapSize` sozinho **não é suficiente** (só cobre o heap do V8; o interpretador Python e toda `Surface`/`Font` do pygame vivem na memória linear do próprio módulo WASM, um espaço completamente separado que aquela chamada nunca vê) - o sinal que importa é o RSS real do processo renderer do Chromium (`SystemInfo.getProcessInfo` via CDP + `/proc/<pid>/status`).

Achados, mesma classe de bug do `font()` original - `Surface`s recriadas do zero todo frame em vez de cacheadas, em código que roda incondicionalmente, sessão inteira:

- **`audio.SoundButton`/`input_system.FullscreenButton`** - desenhados em TODA tela (menu e gameplay), todo frame, sem exceção (`GameStateManager.draw()` chama os dois incondicionalmente). Só 2 estados possíveis cada (mudo/não-mudo, tela-cheia/não) - cacheados por esse estado.
- **`input_system.VirtualJoystick`/`VirtualButton`** (controles de toque mobile) - uma vez que `touch_active` vira `True` ele NUNCA volta a `False` pelo resto da sessão (flag de mão única, Stage K24), então ~12 desses (joystick + ataque/dash/picareta/pausa/3 magias/3 itens/debug) desenhavam Surfaces novas todo frame, pra sempre, no mobile inteiro. Base do joystick e ícone/label de cada botão são 100% estáticos por instância (cacheados uma vez); a única parte de fato dinâmica (o "flash" de alpha ao pressionar) ficou limitada a um cache bem pequeno.
- **`MenuState`'s `Star`/glow do título** - 80 partículas + o brilho atrás do título, ambos recriados todo frame só no menu (o `glow_r` que insinuava variação nem chegava a ser usado no desenho real - morto).
- **O verdadeiro maior contribuinte, achado só com o soak test de coop**: o balão de chat (`Player`/`RemotePlayer._draw_chat_bubble()`, Stage L14) e os chips de debuff (`Player._draw_status_chips()`, mais exercitados agora que Traiçoeiro/Homicida existem) recriavam a `Surface` inteira todo frame enquanto ativos - um soak de single-player (sem chat) não achou nada de anormal, mas o mesmo teste numa room coop de verdade, com chat acontecendo a cada poucos segundos, mostrou memória combinada subindo de forma quase perfeitamente linear (~140 KB/s, ~500MB/hora extrapolado). Corrigido cacheando o balão por TEXTO (só muda quando chega uma mensagem nova) e o chip por `(effect_id, segundos restantes)` (cresce no máximo até a duração do efeito, nunca sem limite).

Confirmado por comparação direta antes/depois com o mesmo soak de coop: o crescimento linear sumiu, virou flutuação sem tendência clara dentro de uma banda pequena. Nenhum teste (single-player ou coop, antes ou depois da correção) chegou a travar de verdade dentro da janela testada (minutos, não horas) - as sondas de responsividade (round-trip de um `page.evaluate()` trivial) nunca estouraram o timeout de 5s em nenhuma rodada, então o que os soak tests realmente comprovam é a *tendência* de memória, não um travamento reproduzido ao vivo.

## Correções pós-playtest coop (Stage L15/L16 + polimento de UI/balanceamento)

Lote de correções depois de um playtest real com 2 jogadores. Achados de sync do coop primeiro (os dois mais sérios), seguidos dos ajustes menores de UI/balanceamento pedidos junto.

**L15 — Quebra de bloco e chave escondida não sincronizavam.** `Level.try_break_tile()`/`_break_block()` nunca tinham NENHUM transporte de rede — cada cliente (host e guest) mutava seu próprio `layout`/`_block_hits` isoladamente, então um bloco que um jogador quebrava continuava sólido pra sempre na tela do outro (achado exatamente assim num playtest: "as pedras que ele quebrava... continuavam existindo pra mim"). Corrigido com um evento **simétrico** (não host-autoritativo como o combate — quebrar um bloco não tem risco real de conflito, diferente de HP de inimigo): quem quer que complete a quebra localmente manda `{"type": "block_broken", "col", "row"}`, o outro lado aplica `_break_block(..., roll_drop=False)` (idempotente, e sem rolar um segundo drop independente pro mesmo bloco). A chave escondida ganhou o mesmo tratamento (`{"type": "key_found"}`) — sem isso, quem achasse a chave abriria a saída só pra si.

**L16 — Guest nunca conseguia ferir/matar um inimigo.** O broadcast host→guest de HP/`alive` de `Enemy`/`Boss` (L6) sempre funcionou certo — o gap real, documentado no próprio código como um "incremento futuro" desde a L6, era que **todo golpe do guest contra um inimigo/boss era um no-op silencioso** (`game/level.py`/`game/states.py` checavam `network_follower` e simplesmente não chamavam `take_damage()`). Do lado do guest isso parece exatamente "monstro fica parado" — o host mata normalmente e sincroniza certo, mas o guest não consegue matar nada, mesmo acertando. Corrigido estendendo o mesmo padrão host-autoritativo (não uma arquitetura nova): nos 4 pontos de combate (melee comum, melee de boss, contato de Dash, projétil), quando o alvo é um follower, o cliente manda `{"type": "hit_request", "enemy_id"/"boss", "damage", ...}` já com o dano calculado localmente; só o host aplica de verdade e credita o kill; o broadcast de `"enemies"` que já roda a ~12Hz devolve o resultado pro guest automaticamente, sem precisar de nenhuma mensagem de saída nova.

**Bug real encontrado testando a própria correção (não um achado de playtest, um erro cometido e pego na hora):** uma melhoria bônus junto com L15 — trocar `random.choice()` por um `random.Random(level_num)` dedicado, pra host e guest concordarem na localização da chave escondida sem precisar de mensagem nova — quebrava o combate PvP inteiro (golpes paravam de registrar depois de poucas rodadas, sem erro visível em lugar nenhum, nem console do navegador). Isolado por bisecção sistemática com `tools/coop_harness.py` (revertendo arquivo por arquivo, depois linha por linha, até achar a exata) - a causa raiz exata nunca ficou 100% clara mesmo instrumentando com traces, então a melhoria foi revertida por completo (`_pick_key_tile()` voltou a usar `random.choice()` puro) em vez de mantida sem entendimento total do porquê. Chave escondida pode, raramente, cair em tiles diferentes pro host e o guest — quirk pré-existente, fora do escopo desta rodada.

**`tools/coop_harness.py` ganhou uma segunda sessão de teste** (`run_world_sync_test`, ao lado da `run_test` original — a sequência de boot/join comum foi extraída pra `_connect_coop_session()` pra não duplicar essa lógica) confirmando L15/L16 via tráfego de rede real: um bloco quebrado pelo guest chega como `"block_broken"` no host, e um ataque do guest contra um `Enemy` chega como `"hit_request"` no host **e** o HP daquele inimigo cai de verdade num snapshot `"enemies"` seguinte (não só que o guest tentou).

**Ajustes menores no mesmo lote:**
- **Tooltip com fundo transparente confundia visualmente** — `draw_tooltip()` (`game/ui.py`) tinha alpha 235/255, quase opaco mas ainda perceptível contra fundos ocupados; virou preto sólido, mesma mudança vale pra hotbar/inventário/barras/chips (todos passam pela mesma função).
- **Selecionar um 4º item pra hotbar (máx. 3) não fazia nada** — `ItemsOverlay._toggle_hotbar()` (`game/merchant.py`) agora remove o mais antigo (`hotbar_items.pop(0)`) antes de adicionar o novo, em vez de silenciosamente ignorar o clique.
- **Recuo (knockback) podia lançar o jogador pra fora do mapa** — não era falta de checagem de colisão (o branch de knockback já usava os mesmos resolvedores de parede do movimento normal), e sim *tunneling*: a resolução discreta só checa sobreposição *depois* de mover, e um `dt` anormal (engasgo/GC) pode deslocar mais que um tile inteiro sem nunca sobrepor a parede. Corrigido com um clamp de segurança no fim de `Player.update()` (`self.x`/`self.y` presos aos limites do mapa), mais barato e seguro que reescrever a colisão pra "swept" em todo movimento/dash.
- **3 números novos no painel de balanceamento** (`player.pickaxe.cooldown`, `stats.base_attack_cooldown`, `level.respawn_interval`) — os dois primeiros já eram constantes soltas (`PICKAXE_COOLDOWN` em `game/player.py`) ou viraram uma (`BASE_ATTACK_COOLDOWN` extraído do literal `0.45` inline em `StatBlock.attack_cooldown`); `RESPAWN_INTERVAL` (`game/level.py`) idem, era um literal `6.0` direto no `__init__`. Os três reaproveitam o mesmo padrão `setattr` no módulo (`game/balance_config.py`) que `player.dash.*`/`stats.*` já usavam - nenhuma UI nova, aparecem sozinhos no painel admin por já cair nas categorias/abas existentes.

## 2ª rodada de correções pós-playtest coop

Um novo playtest real reportou os MESMOS sintomas de sync do lote anterior (pedras/monstros não sumindo pro outro lado, além de magias nunca acertando outro jogador) mesmo já com L15/L16 no ar. Investigação por leitura de código, não suposição: `block_broken`/`key_found` já eram simétricos (gate só em `net_coop.is_connected()`, não em host/guest) e o pipeline `hit_request` de melee/dash/Bola de Fogo também já estava correto - nenhum dos dois tinha um bug estrutural.

**Causa real: `_cast_frost_nova()` nunca foi migrada pro pipeline `hit_request`.** Diferente do loop de projétil da Bola de Fogo (que já checava `network_follower` e mandava `hit_request`), a Nova de Gelo aplicava `target.take_damage()` direto em QUALQUER alvo - contra um `Enemy`/`Boss` do host, o dano de um guest ficava só local e revertia no snapshot seguinte (o mesmo "monstro parece congelado" do lote anterior, só que causado por magia em vez de física de rede). A mesma função também nunca testava `self.remote_players` nenhuma vez - um caster de Nova de Gelo em cima de outro jogador não causava dano nenhum nele, e o loop de projétil da Bola de Fogo tinha a mesma lacuna (só testava `cast_targets`, que é só inimigo/boss). Os 3 corrigidos juntos (mesma área de código): Nova de Gelo ganhou o `network_follower`/`hit_request` que faltava (incluindo o `credit_kill()` que nunca era chamado - kills de Nova de Gelo nem em single-player davam XP/ouro, bug pré-existente sem relação com coop), e as duas magias passaram a testar `remote_players` e mandar `{"type": "player_hit", ...}` no mesmo formato que o melee (`_handle_pvp_attacks`) já usa.

**`tools/coop_harness.py` ganhou uma 3ª sessão de teste** (`run_world_sync_reverse_test`) especificamente pra cobrir a direção HOST→GUEST (a original `run_world_sync_test` só testava GUEST→HOST) - confirmou que bloco quebrado pelo host e kill por melee do host chegam corretos no guest (a suspeita de bug estrutural de sync não se confirmou), e uma nova asserção provou a Nova de Gelo acertando o outro jogador via tráfego de WebSocket real. `_make_save()` ganhou um parâmetro `attributes` opcional (INT/SAB do personagem de teste padrão são baixos demais pra sequer desbloquear Nova de Gelo).

**Atribuição de kill errada em coop, achada montando o placar da tela de vitória abaixo:** todo kill resolvido via `hit_request` sempre creditava `player.kills` de quem RESOLVE (sempre o host), nunca de quem de fato acertou o golpe (o guest). `Level.credit_kill()` ganhou um `killer_id` opcional - quando é de outra pessoa, manda `{"type": "kill_credit", "player_id", "etype"}` (mesmo padrão self-filtering de `player_hit`) em vez de incrementar localmente; os 5 pontos que mandam `hit_request` (melee comum, melee de boss, Dash, projétil, Nova de Gelo) agora incluem `player_id` de quem golpeou.

### Tela de vitória de fase, redesenhada pra coop

`StageCompleteState` recebia `coop`/`remote_players` desde a L6 mas nunca usava - mostrava sempre só o resumo solo, mesmo com outro jogador na room. Agora, em coop, vira um placar com uma coluna por jogador (portrait real via `create_player_sprite(..., profession)`, nome, kills/ouro/XP da fase, barra de XP individual, e um destaque de quem achou a chave escondida). Kills/ouro/XP de um `RemotePlayer` não existem no cliente de quem está olhando (cada um só sabe do próprio `Player`) - resolvido com um auto-report: ao entrar nesta tela, cada cliente manda `{"type": "level_complete_stats", "player_id", "kills", "gold", "xp", "level", "xp_frac"}` pra room, e a tela mostra "..." pra quem ainda não chegou (rede/quem demora mais pra sair da fase). Isso exigiu que `StageCompleteState.update()` passasse a chamar `net_coop.poll_messages()` - antes só `GameplayState` fazia isso; sem isso uma mensagem chegando enquanto esta tela está aberta ficaria na fila até a PRÓXIMA fase carregar, tarde demais pra aparecer aqui.

**Bônus de XP por achar a chave**: `Level.key_finder_id` guarda quem cavou a chave (local, ou via `player_id` na mensagem `key_found` - só a 1ª vez conta). Na transição de fase, se `key_finder_id` bater com o próprio `net_coop.get_player_id()`, ganha `stats.KEY_FINDER_XP_BONUS` (padrão 40, novo tunable `stats.key_finder_xp_bonus` no painel de balanceamento) antes do `xp_gained` da tela ser calculado - o número mostrado já reflete o bônus.

### Easter egg: mapa do tesouro

Ao limpar a leva ORIGINAL de inimigos de uma fase (não as ondas de respawn depois dela - trava de disparo único, `Level._first_wave_map_shown`), o herói ergue um mapa do tesouro sobre a cabeça com o mesmo som de sucesso de achar a chave, e 10 marcações decorativas ("X" vermelho sobre um pergaminho, `create_treasure_mark_icon()`) aparecem espalhadas pela fase em tiles de piso aleatórios - só visual, sem hitbox, nenhum item entra no inventário. Reaproveita a mesma infra da pose da chave (`Player.key_pose_timer`/`_draw_key_pose()`), generalizada com um `pose_icon` ("key" ou "map") em vez de um ícone fixo. Roda em cada cliente de forma independente (sem mensagem de rede nova) - a mesma condição "leva original zerada" já é derivada do estado de inimigos que cada lado já enxerga (local no host, via snapshot no guest), e as posições em si são só decorativas (mesma tolerância a leve divergência host/guest que a própria localização da chave já aceita hoje, ver "Bug real encontrado" acima).

### Painel de balanceamento: timeout no login

Usuário reportou o login travando. Testado direto contra produção (`curl`) - `POST /admin/login`, `GET /admin/balance`, `/appearance*` todos responderam corretos em <1s, nenhuma lentidão do lado do servidor reproduzida. Sem uma causa raiz clara pra corrigir, `doLogin()`/`loadBalance()` (`backend/app/main.py`, JS embutido do painel `/admin`) ganharam um timeout explícito de 10s (`AbortController`) com uma mensagem "Tempo esgotado" distinta de "Senha invalida" - pelo menos da próxima vez que acontecer, dá um sinal diagnosticável em vez de parecer simplesmente travado.

## Leva de conteúdo: kits de classe (Estágio M1)

Primeiro estágio de uma leva grande pedida pelo usuário - ataque básico + kit de 3 magias (ofensiva/utilitária/ultimate) por profissão, um sistema de família de monstros, e um redesenho de fases. Pesquisa prévia (ver `.claude/plans/jiggly-whistling-valley.md`) achou que 11 das 45 magias pedidas precisavam de sistemas novos que não existiam (taunt, teleporte, stun, root, totem/pet aliado, armadilha, execução, combo) - o usuário confirmou explicitamente construir esses sistemas de verdade em vez de reaproveitar algo existente disfarçado. Este estágio (M1) é só a arquitetura + prova de conceito; kits autorais completos pras 13 profissões restantes e a leva de monstros/fases ficam pra sessões futuras.

**Arquitetura nova**: `game/class_kits.py`'s `CLASS_KITS` (profissão -> ataque básico + trio de magias) substitui o antigo esquema onde toda profissão compartilhava a mesma lista global de magias (`game/spells.py`'s antigo `ORDER`), gateada por limiar de atributo (`meets_requirements()`, removido) - agora "ter a profissão com aquela magia no kit" é o único requisito, sem checagem redundante (a profissão já É derivada da mesma distribuição de atributos). 3 pontos que assumiam a lista global fixa por nome literal (`Player._spell_action`, `game/paperdoll.py`'s Magias tab, `game/input_system.py`'s arco de magias móvel) viraram posicionais contra `spells_for(player.profession)`, recalculado a cada leitura (um respec troca o kit inteiro em tempo real). **Achado ao vivo rodando o harness de coop depois desta mudança**: o `DEFAULT_KIT` (as 6 profissões sem kit autoral ainda) inicialmente reordenou Nova de Gelo pra tecla R em vez de Q (a posição que ela sempre teve) - quebrou o teste de PvP do harness, que pressiona "q" esperando ela. Corrigido preservando o F/Q/R exato de antes desta leva pro trio antigo (fireball/frost_nova/healing_light), mesmo que isso rotule Nova de Gelo como "utility" internamente (só a posição importa, não o rótulo).

**Guerreiro e Assassino** têm kit 100% autoral: ataque básico próprio (`basic_attack_for()` aplica multiplicadores de alcance/tamanho/velocidade/dano em cima do hitbox universal de sempre - sem geometria nova, o motor de colisão não tem conceito de arco/cone) e 3 magias reais cada. As outras 7 profissões com alguma magia nova (Cavaleiro/Campeão/Druida/Xamã/Ranger/Paladino/Duelista) recebem só a magia de prova-de-conceito do sistema novo delas, preenchendo as outras 2 posições com o trio antigo.

**8 sistemas novos** (cada um com uma magia de prova-de-conceito):
- **Taunt** (Provocação/Cavaleiro): sem tabela de ameaça real (`Enemy.update()` só enxerga um `Player` por vez, mesmo em coop) - escopado pra um status novo "provoked" (`game/status_effects.py`, único status com `speed_mult` > 1.0 de propósito) que deixa inimigos próximos mais agressivos, não uma reatribuição de alvo de verdade.
- **Stun** (Impacto Sísmico/Campeão) e **Root** (Raízes Prendentes/Druida): os dois reusam o eixo `speed_mult` já existente no valor extremo 0.0 (sem eixo novo) - a diferença real é que `Enemy.update()` (`game/enemy.py`) checa `self.status.has("stun")` especificamente pra também travar o ataque, não só o movimento.
- **Teleporte** (Passo Sombrio/Assassino): `Player.try_teleport()`, nova - anda o salto pra trás em passos curtos até achar uma posição final sem colisão, ao contrário do Dash (que anda de verdade, step a step).
- **Totem/pet** (Totem Curativo/Xamã) e **Armadilha** (Ranger): duas entidades persistentes novas em `game/level.py` (`HealTotem`/`Trap`), mesmo padrão de `GoldDrop`/`Puddle` já existentes - a primeira vez que uma entidade "do lado do jogador" (não hostil) fica no mundo. Local-only por enquanto, sem sync de rede.
- **Execução** (Julgamento/Paladino): fórmula nova (bônus de dano proporcional à vida FALTANTE do alvo mais próximo, até +150%).
- **Combo** (Dança das Lâminas/Duelista): reinterpretado como 3 pulsos de dano reais num cast só, não um contador persistente entre ataques normais (exigiria instrumentar todo golpe do jogo - fora de escopo desta leva).

**Verificação**: `tools/coop_harness.py` completo (3 sessões, incluindo a checagem de PvP de Nova de Gelo da rodada anterior) roda limpo depois do fix acima. Script dedicado (`verify_class_kits.py`, scratchpad) forçou as 9 profissões com conteúdo novo via o "Profissão (forçar)" do painel de debug e conjurou cada magia nova ao vivo - zero erros de página, ícones novos (`create_spell_icon()`, 13 entradas) renderizando certo na hotbar.

## Leva de conteúdo: kits de classe completos (Estágio M2)

Continuação do Estágio M1 - as 13 profissões que antes tinham só uma magia de prova-de-conceito (ou nada) ganharam kit 100% autoral: ataque básico próprio + 3 magias reais cada, 31 magias novas no total (47 no jogo inteiro, contra as 3 originais). Nenhum sistema novo nesta leva - as 8 primitivas do Estágio M1 (taunt/stun/root/teleporte/totem/armadilha/execução/combo) já cobriam o que precisava existir; M2 foi só conteúdo em cima da arquitetura já pronta.

**Redução de duplicação**: em vez de 31 métodos `_cast_<id>` bespoke (o que o Estágio M1 fez pras primeiras 16 magias, já com bastante repetição entre elas), toda magia nova do M2 encaixa num dos 4 "formatos" genéricos - `_cast_generic_projectile` (projétil reto, com `status_effect`/`status_chance` opcionais lidos do próprio dict da magia - Raio de Gelo, Correntes Malditas, Chama Negra, etc.), `_cast_generic_aoe` (a mesma área-ao-redor-do-jogador de `_aoe_around_player`, agora parametrizada por `radius`/`dtype`/`status_to_enemy` no dict da magia em vez de hardcoded por spell), `_cast_generic_buff` (`status.apply(spell["buff_id"])`), `_cast_generic_heal`. Cada `_cast_<id>` de fato vira 1 linha delegando pro genérico certo - o `getattr(self, f"_cast_{spell_id}")` do `_attempt_cast()` (Estágio M1) continua funcionando sem mudar nada, já que o NOME do método é o que importa pro dispatch, não o corpo.

**Achado ao vivo, reaproveitado desta vez em vez de descoberto de novo**: o loop de colisão de projétil do jogador (`game/states.py`) nunca aplicava `Projectile.status_effect`/`status_chance` no alvo atingido, mesmo esses campos já existindo na classe (usados por projéteis de boss/inimigo) - Raio de Gelo/Correntes Malditas precisavam disso pra aplicar Lentidão/Fraqueza no impacto, então o loop ganhou essa checagem que faltava (mesma classe de gap que a Nova de Gelo já teve na 2ª rodada de bugfix coop, só que em outro lugar do código).

Ataque básico de todo caster puro (Mago/Feiticeiro/Arcanista/etc) continua sendo o hitbox físico universal re-escalado (mais fraco, um pouco mais longo) - um "ataque básico mágico" de verdade (com seu próprio formato de dano/visual) ficou fora do escopo, o dano real de um caster sempre vem das magias.

**Verificação**: harness de coop completo roda limpo (nenhuma regressão nova). Script (`verify_all_kits.py`, scratchpad) cicla pelas 15 profissões em sequência via o painel de debug, conjurando ataque básico + as 3 magias de cada uma ao vivo - zero erros de página, ícones novos (31, `create_spell_icon()`) renderizando certo, barras de cooldown corretas nos 3 slots.

## Leva de conteúdo: famílias de monstro (Estágio N)

20 famílias × 3 variações cada, seguindo a decisão já confirmada com o usuário: cada tier ganha stats/sprite/flavor de ataque PRÓPRIOS (entrada independente em `ENEMY_ARCHETYPES`/`BASE_XP`/`GOLD_DROPS`/`ENEMY_FLAVOR`/`ENEMY_SPRITES`, nunca um multiplicador em cima de uma base compartilhada) - mesma disciplina que já levou à aposentadoria dos recolors-só-de-cor duas vezes antes neste projeto.

**Roteiro da família resolvido** (pesquisa prévia tinha achado ambiguidades - aqui estão as decisões tomadas): 7 famílias reaproveitam 1-3 monstros já existentes como algum dos tiers (Skeleton = skeleton→dark_skeleton→death_knight; Saurian = serpente→lyzardman→saurio_ancestral [novo]; Spider/Goblin/Demon/Golem/Zombie/Witch = tier 1 existente + 2 novos; Brute = troll→ogro→ogro_ancião [novo], fundindo o par troll/ogro que já compartilhava rig). As outras 12 famílias (Rat/Toad/Minotaur/Wolf/Bear/Orc/Drake/Vampire/Ghost/Slime/Crab/Werewolf) são 100% novas, 3 monstros cada. `dark_knight`/`treant`/`dark_horse`/`chimera`/`verme`/`fire_hound` seguem standalone (bespoke, sem família clara) - `verme` especificamente foi separado de `serpente` de propósito, já que os dois compartilhavam a mesma rig só diferenciados por cor (um recolor-pair que passou despercebido nas duas aposentadorias anteriores).

**49 monstros novos** (89 no total do jogo, contra 20 antes). Números de combate calibrados por um template de 3 tiers (fraco/médio/forte, aproximando a faixa que os 20 arquétipos originais já cobrem) mais um delta temático por família (Urso é lento e resistente, Rato é rápido e frágil, Fantasma tem DES alta e VIG baixa, etc.) - sem `balance_sim.py` pra validar com precisão ainda, então os números são um ponto de partida razoável, ajustável pelo painel de balanceamento.

**Sprites**: sistema de *overlay* novo (`game/assets.py`'s `_OVERLAY_PAINTERS`) - 4 overlays genéricos (`spikes`/`crown`/`glow_eyes`/`armor`) desenhados EM CIMA da rig base depois dela, reaproveitados entre várias famílias (o tier 2 de uma família ganha 1 overlay, o tier 3 ganha 2) em vez de uma função pintora bespoke por tier - a mesma abordagem que a pesquisa prévia já tinha validado ("tier 2/3 pode reusar a mesma rig com recolor + ajuste de silhueta"), só que a "ajuste de silhueta" virou um sistema reutilizável em vez de coordenadas hardcoded editadas manualmente em cada rig. As 12 famílias 100% novas ganharam 12 rigs novas (`_paint_rat`/`_paint_toad`/`_paint_minotaur`/`_paint_wolf`/`_paint_bear`/`_paint_orc`/`_paint_drake`/`_paint_vampire`/`_paint_ghost`/`_paint_slime`/`_paint_crab`/`_paint_werewolf`), mesmo estilo de coordenadas manuais num grid 16×16 que toda rig já usava.

**ENEMY_FLAVOR** (comportamento de ataque) usa as 5 primitivas já existentes (melee_status/ranged/ranged_shape/puddles/atk_cd) sem nenhum código novo - tier 1 de cada família é só melee (`melee_status` fraco), tier 2 ganha `melee_status` mais forte (às vezes com `ranged`), tier 3 soma `ranged`+`ranged_shape` - confirma que essas 5 primitivas já bastam pra diferenciar 3 tiers sem precisar de mecânica nova.

**Fora do escopo deste estágio, de propósito**: nenhum dos 49 monstros novos foi colocado em `LEVEL_MAPS` ainda - isso é trabalho do Estágio O (redesenho de fases), que decide quantas fases/atos fazem sentido pra acomodar a variedade nova. Este estágio só constrói as identidades (stats/sprite/comportamento/balanceamento), prontas pra serem distribuídas.

**Verificação**: cada um dos 69 arquétipos (20 originais + 49 novos) foi instanciado como `Enemy` de verdade (não só os dicts crus) e teve `draw()`/`update()` chamados sem erro, headless. Harness de coop completo roda limpo (nenhuma regressão - `LEVEL_MAPS` não mudou, então as fases existentes se comportam exatamente como antes).

## Leva de conteúdo: fases herdam as famílias de monstro (Estágio O)

Decisão principal deste estágio: em vez de autorar fases/atos novos do zero (cada fase existente tem um layout ASCII desenhado à mão, `player_start`/`exit`/pool de tiles pra chave - um trabalho de design real por fase, não mecânico) pra dar "canvas" aos 49 monstros novos do Estágio N, as **9 fases de combate já existentes passaram a sortear de um pool de família maior**, e o próprio dial de dificuldade que já existia (`game/difficulty.py`, Normal→Inferno) virou o mecanismo que decide QUAL tier de cada família aparece.

**Como funciona**: `LEVEL_MAPS`'s campo `"enemies"` (`game/level.py`) trocou de listar etypes concretos (`"skeleton"`) para listar nomes de família (`game/stats.py`'s `MONSTER_FAMILIES`, ex: `"skeleton"` continua funcionando igual pra essa família específica, mas `"orc"`/`"lobo"`/`"urso"` etc. agora resolvem pra um dos 3 tiers). `resolve_family(nome, tier_index)` decide o etype concreto na construção da fase (`Level._build()`); `difficulty_tier_index(order)` mapeia a ordem 0-4 das 5 dificuldades pra 3 tiers (Normal/Difícil → tier fraco, Muito Difícil/Pesadelo → médio, Inferno → forte) - **jogar numa dificuldade mais alta agora mostra monstros genuinamente diferentes** (visual e stats), não só números maiores via `ml_bonus` (que continua se aplicando por cima, inalterado). Um nome que não é uma família conhecida (os 6 monstros standalone - `dark_knight`/`treant`/`dark_horse`/`chimera`/`verme`/`fire_hound`) resolve pra si mesmo em qualquer tier, mesmo comportamento de sempre - nenhum `LEVEL_MAPS` antigo quebraria mesmo sem essa mudança.

**Roster novo por fase** (cada fase de combate ganhou 2-7 famílias, contra 2-3 etypes fixos antes): Floresta Encantada ficou DELIBERADAMENTE perto do original (só skeleton/goblin) - ver "Achado ao vivo" abaixo; Ruínas do Deserto herdou Rato/Lobo/Urso além de Orc/Caranguejo (fase 2 nunca é exercida pelo coop harness, segura pra variedade mais rápida/agressiva); Masmorra das Sombras trocou os goblins repetidos por Orc, de propósito - é a fase logo antes do boss Orc Warlord, que antes não tinha NENHUM orc comum na campanha pra dar contexto narrativo antes do chefe; Pântano Sombrio ganhou Geleia; Torre Amaldiçoada ganhou Fantasma/Minotauro; Cripta Perdida ganhou Vampiro; Salão dos Ecos ganhou Fantasma; Abismo de Cinzas ganhou Drake; Corredor Final ganhou Lobisomem. Todas as 20 famílias e os 6 monstros standalone aparecem em pelo menos uma fase.
